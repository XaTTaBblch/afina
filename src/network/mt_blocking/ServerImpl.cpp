...
// See Server.h
ServerImpl::ServerImpl(std::shared_ptr<Afina::Storage> ps, std::shared_ptr<Logging::Service> pl) : Server(ps, pl), _cur_workers(0) {}
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        if (_cur_workers < _max_workers) {
            ++_cur_workers;
            std::thread(&ServerImpl::OnConnect, this, client_socket).detach();
        } else {
            _logger->debug("Too many threads");
            close(client_socket);
        }
...
void ServerImpl::OnConnect(int client_socket) {
    ...
    try {
        ...
        while ((running || command_to_execute) && (readed_bytes = read(client_socket, client_buffer, sizeof(client_buffer))) > 0) {
